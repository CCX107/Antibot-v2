...
from anti_bot_utils import (  # 逐行说明：导入稳定、可反序列化的特征构造类
    FeatureColumnSelector,
    RESOLUTION_Z_FEATURE,
    UnifiedUserBehaviorCleaner,
    UnifiedUserBehaviorCleanerWithResolutionShareZ,
)
...
RAW_EVENT_REQUIRED_COLUMNS = ["date", "distinct_id", "$screen_width", "$screen_height"]
SEEK_REPORT_JOINED_PATH = str(BASE_DIR / "seek_report_joined.parquet")  # 逐行说明：设置 SEEK_REPORT_JOINED_PATH 的值
REDTEAM_RESULTS_PATH = BASE_DIR / "redteam_results_resolution_v3.json"
...
MODEL_CONFIGS = {  # 逐行说明：设置 MODEL_CONFIGS 的值
    "原始模型": {  # 逐行说明：配置字典字段
        "model_path": str(BASE_DIR / get_config_value("IFOREST_MODEL_FILE", "antibot_pipeline_v3_resolution_zscore.pkl")),  # 逐行说明：配置字典字段
        "features_path": str(BASE_DIR / get_config_value("IFOREST_FEATURES_FILE", "features_all_iforest_v3_resolution_zscore.parquet")),  # 逐行说明：配置字典字段
        "prediction_mode": "anomaly",  # 逐行说明：配置字典字段
        "required_columns": ["model_anomaly", "final_time_risk", RESOLUTION_Z_FEATURE],  # 逐行说明：配置字典字段
    },  # 逐行说明：传入参数或列表项
    "XGB mixed 新模型": {  # 逐行说明：配置字典字段
        "model_path": str(BASE_DIR / get_config_value("XGB_MODEL_FILE", "xgb_model_mixed_v3_resolution_zscore.pkl")),  # 逐行说明：配置字典字段
        "features_path": str(BASE_DIR / get_config_value("XGB_FEATURES_FILE", "features_all_xgb_mixed_v3_resolution_zscore.parquet")),  # 逐行说明：配置字典字段
        "prediction_mode": "binary_bot",  # 逐行说明：配置字典字段
        "required_columns": ["model_anomaly", "xgb_bot_prob", RESOLUTION_Z_FEATURE],  # 逐行说明：配置字典字段
    },  # 逐行说明：传入参数或列表项
}  # 逐行说明：结束当前结构
...
REDTEAM_MODEL_VERSION = "内置旧结果"
if REDTEAM_RESULTS_PATH.exists():
    try:
        redteam_payload = json.loads(REDTEAM_RESULTS_PATH.read_text(encoding="utf-8"))
        REDTEAM_ATTACK_RESULTS = redteam_payload["attack_results"]
        REDTEAM_NORMAL_RESULTS = redteam_payload["normal_results"]
        REDTEAM_MODEL_VERSION = redteam_payload.get("model_version", "未知版本")
    except (KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
        st.warning(f"分辨率模型压测结果无法读取，暂时展示内置旧结果：{exc}")
...
def build_redteam_tables():  # 逐行说明：定义 build_redteam_tables 函数
    ...
    for df in [attack_df, normal_df]:  # 逐行说明：开始循环遍历数据
        for col in ["resolution_z_mean", "resolution_z_p95"]:
...
def _core_predict_logic(df: pd.DataFrame, model_path: str, prediction_mode: str) -> pd.DataFrame:  # 逐行说明：定义 _core_predict_logic 函数
    ...
    missing_cols = [col for col in model_cols if col not in features.columns]
    if missing_cols:
        raise ValueError(
            f"Cleaner 未生成模型需要的特征：{missing_cols}。"
            "请检查模型版本、原始 parquet 字段和 Cleaner 是否一致。"
        )
...
def _score_existing_features(features: pd.DataFrame, model_path: str, prediction_mode: str) -> pd.DataFrame:
    """复用 IForest 用户日特征，避免 XGBoost 再跑一次相同 Cleaner。"""
    model_obj = load_model(model_path)
    model = model_obj.named_steps["model"] if hasattr(model_obj, "named_steps") else model_obj
    output = features.copy()
    model_cols = _model_feature_columns(model, output)
    missing_cols = [col for col in model_cols if col not in output.columns]
    if missing_cols:
        raise ValueError(f"共享 IForest 特征缓存缺少 XGBoost 特征：{missing_cols}")
    model_input = output[model_cols]
    raw_pred = _predict_with_model(model, model_input)
    output["model_anomaly"] = _normalize_model_anomaly(raw_pred, prediction_mode).values
    if prediction_mode == "binary_bot":
        output["xgb_bot_prob"] = _predict_bot_probability(model, model_input).values
    return output
...
def build_and_store_features(  # 逐行说明：定义 build_and_store_features 函数
    ...
    if prediction_mode == "binary_bot" and os.path.exists(IFOREST_FEATURES_PATH):
        shared_features = pd.read_parquet(IFOREST_FEATURES_PATH)
        features = _score_existing_features(shared_features, model_path, prediction_mode)
    else:
...
def build_and_store_incremental_features(
    ...
    if prediction_mode == "binary_bot" and os.path.exists(IFOREST_FEATURES_PATH):
        normalized_for_query = sorted({pd.to_datetime(d).strftime("%Y-%m-%d") for d in replaced_dates})
        date_literals = ", ".join(f"DATE '{d}'" for d in normalized_for_query)
        shared_conn = duckdb.connect()
        try:
            shared_features = shared_conn.execute(f"""
                SELECT * FROM read_parquet('{str(IFOREST_FEATURES_PATH).replace("'", "''")}')
                WHERE TRY_CAST(date AS DATE) IN ({date_literals})
            """).df()
        finally:
            shared_conn.close()
        if shared_features.empty:
            raise RuntimeError("XGBoost 增量预测找不到对应日期的 IForest 共享特征缓存。")
        features = _score_existing_features(shared_features, model_path, prediction_mode)
    else:
...
def local_raw_cache_status(start_s: str, end_s: str) -> tuple[bool, str, int]:
    ...
    if not _feature_store_has_columns(DATA_PATH, RAW_EVENT_REQUIRED_COLUMNS):
        return False, "本地原始 parquet 缺少分辨率字段，需要按当前窗口重新拉取", 0
...
def run_local_sync():  # 逐行说明：定义 run_local_sync 函数
    ...
    # 2. 检查本地数据进度
    raw_schema_ok = _feature_store_has_columns(DATA_PATH, RAW_EVENT_REQUIRED_COLUMNS)
    if os.path.exists(DATA_PATH) and raw_schema_ok:  # 逐行说明：只有新字段完整时才允许增量复用
    ...
    else:  # 逐行说明：文件不存在或旧缓存缺少分辨率字段时重拉完整窗口
...
con.execute(f"""
    ...
    SELECT
        ...
        i.final_time_risk,
        i.resolution_share_positive_zscore,
...
with tab_redteam:  # 逐行说明：进入上下文管理代码块
    ...
    st.caption(f"压测模型版本：{REDTEAM_MODEL_VERSION}；结果文件：{REDTEAM_RESULTS_PATH.name}")
    ...
    summary_display = redteam_summary_df[[  # 逐行说明：设置 summary_display 的值
        ...
        "normal_pool_direct_hit_rate", "normal_pool_xgb_added_hit_rate", "resolution_z_mean", "resolution_z_p95",
        "both_black", "xgb_high_new", "xgb_mid_new", "xgb_edge", "verdict"
        ...
        "resolution_z_mean": "攻击样本分辨率Z均值", "resolution_z_p95": "攻击样本分辨率Z P95",
